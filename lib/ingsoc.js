...
const BigBrother = (() => {
    ...
    const callbacks = {
        ...
        listenToNode: ({thinkpol, observable, observedProperty, node, nodeProperty}) => {
            node.addEventListener(nodeProperty, (event) => {
                const listener = observable.listeners[observedProperty];
    ...
    class ThoughtPolice {
        ...
        setup(parentEl, nodeUUIDs, childUUIDs) {
            ...
            const instance = this.personnelDetails(false);
            this.parseDOM(parentEl, nodeUUIDs, instance);
        }

        bindListeners() {
            const instance = this.personnelDetails();
            for (const name in instance.listeners) {
                const listener = instance.listeners[name];
                instance.listeners[name] = listener.bind(instance);
            }
        }
